      ...
      <main className="flex flex-col gap-10">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-y-10 md:gap-y-0 md:gap-x-10">
          <div className="col-span-2">
            <h1 className="font-semibold mb-1">{visibility.charAt(0).toUpperCase() + visibility.slice(1)} projects</h1>
            <Suspense fallback={<SkeletonProjectList />}>
              <ManageProjectList page={currentPage} visibility={visibility} query={query} />
            </Suspense>
          </div>
  ...
  return (
    <Card className="w-full col-span-2 h-[40dvh]">
      <div className="w-full h-full flex justify-center items-center">
        <p className="text-lg text-black text-center">Why don't you try making a publication?</p>
      </div>
    </Card>
  )
}
